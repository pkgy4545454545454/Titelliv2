...
async def purchase_training(training_id: str, current_user: dict = Depends(get_current_user)):
    ...
    try:
        ...
        checkout_request = CheckoutSessionRequest(
            ...
            currency="chf",
            success_url=f"{os.environ.get('FRONTEND_URL', 'https://dependency-cleanup-3.preview.emergentagent.com')}/payment/success?session_id={{CHECKOUT_SESSION_ID}}&type=training&training_id={training_id}",
            cancel_url=f"{os.environ.get('FRONTEND_URL', 'https://dependency-cleanup-3.preview.emergentagent.com')}/payment/cancel",
...
async def apply_for_certification(
    ...
    try:
        ...
        session_request = CheckoutSessionRequest(
            ...
            currency="chf",
            quantity=1,
            success_url=f"{os.environ.get('FRONTEND_URL', 'https://dependency-cleanup-3.preview.emergentagent.com')}/enterprise-dashboard?tab=certifications&success=true",
            cancel_url=f"{os.environ.get('FRONTEND_URL', 'https://dependency-cleanup-3.preview.emergentagent.com')}/enterprise-dashboard?tab=certifications&cancelled=true",
            metadata={
                "type": "certification",
...
async def book_expert_service(
    ...
    try:
        ...
        session_request = CheckoutSessionRequest(
            ...
            currency="chf",
            quantity=1,
            success_url=f"{os.environ.get('FRONTEND_URL', 'https://dependency-cleanup-3.preview.emergentagent.com')}/dashboard?success=expert",
            cancel_url=f"{os.environ.get('FRONTEND_URL', 'https://dependency-cleanup-3.preview.emergentagent.com')}/dashboard",
            metadata={
                "type": "expert_service",
